...
const MechanicPanel = ({ user, handleLogout, handleChangePassword, rawHistory = [], queue = [], onStartWork, onComplete, onToggleTask, formatTime }) => {
    ...
    return (
        <div className="p-6 max-w-[1400px] mx-auto animate-fade-in pb-20 text-zinc-900">
            {/* Header Profile */}
            <div className="flex flex-col sm:flex-row justify-between items-center mb-8 bg-zinc-900 p-6 rounded-[2rem] shadow-xl gap-4">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                ...
                <div className="lg:col-span-8 flex flex-col gap-8">
                    ...
                    <section>
                        <div className="flex items-center justify-between mb-4 px-2">
                            <h2 className="text-[11px] font-black uppercase tracking-[0.3em] text-zinc-500 flex items-center gap-2">
                                <div className="w-2 h-2 bg-blue-500 rounded-full animate-pulse"></div> Sedang Dikerjakan
                            </h2>
                            <span className="bg-blue-50 text-blue-600 text-[10px] font-black px-3 py-1 rounded-full">{myActiveJobs.length} Aktif</span>
                        </div>

                        {myActiveJobs.length > 0 ? (
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {myActiveJobs.map(item => (
                                    <div key={item.id} className="bg-white border-2 border-dashed border-blue-500/30 rounded-[2rem] p-6 shadow-sm relative overflow-hidden group">
                                        <div className="absolute top-0 right-0 w-32 h-32 bg-blue-50/50 rounded-bl-full -z-10 transition-transform group-hover:scale-110"></div>
                                        <div className="flex justify-between items-start mb-4">
                                            <div>
                                                <span className="bg-red-600 text-white text-[8px] font-black px-2 py-0.5 rounded-full uppercase tracking-widest mb-2 inline-block shadow-sm">{item.category}</span>
                                            ...
                                            <div className="text-right">
                                                ...
                                                <p className="text-xl font-black text-blue-600 leading-none tabular-nums">
                            ...
                            <div className="bg-zinc-100 border-2 border-dashed border-zinc-200 rounded-[2.5rem] p-16 text-center">
                    ...
                    <section>
                        ...
                        <div className="bg-white rounded-[2.5rem] border border-zinc-100 overflow-hidden shadow-xl shadow-zinc-200/40">
                            {availableQueue.length > 0 ? (
                                <div className="divide-y divide-zinc-50">
                                    {availableQueue.map(item => (
                                        <div key={item.id} className="p-6 hover:bg-zinc-50 transition-all flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                                            <div className="flex items-center gap-4">
                                                <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-white shadow-lg ${item.status === 'menginap' ? 'bg-zinc-900' : 'bg-red-600'}`}>
                                                    {item.status === 'menginap' ? <Shield size={20} fill="white" /> : <Zap size={20} fill="white" />}
                                                </div>
                                                <div>
                                                    <div className="flex items-center gap-2 mb-0.5">
                                                        <h4 className="text-xl font-black text-zinc-900 font-mono leading-none tracking-tight">{item.bk}</h4>
                                                        <span className={`text-[8px] font-black px-2 py-0.5 rounded-full uppercase tracking-widest ${item.status === 'menginap' ? 'bg-zinc-100 text-zinc-400 border border-zinc-200' : 'bg-red-50 text-red-600'}`}>{item.status === 'menginap' ? 'Menginap' : item.category}</span>
                                                    </div>
                                                    <p className="text-[9px] font-black text-zinc-400 uppercase tracking-widest leading-none">{item.tipe}</p>
                                                </div>
                                            </div>
                                            ...
                                            <div className="flex items-center gap-4 sm:border-l sm:pl-8 border-zinc-100">
                                                ...
                                                <button
                                                    ...
                                                    className={`px-8 py-3 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all active:scale-95 shadow-lg flex items-center justify-center gap-2 ${item.status === 'menginap' ? 'bg-orange-500 text-white shadow-orange-200' : 'bg-blue-600 text-white shadow-blue-200'}`}
                                                >
                                                    <Zap size={14} fill="white" /> {item.status === 'menginap' ? 'Lanjutkan Pekerjaan' : 'Mulai Pekerjaan'}
                                                </button>
                                            </div>
                                        </div>
                                    ))}
                ...
                <div className="lg:col-span-4 flex flex-col gap-6">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="bg-white p-6 rounded-[1.5rem] border border-zinc-200 shadow-lg shadow-zinc-200/30">
                            <TrendingUp className="text-blue-500 mb-3" size={20} />
                            <p className="text-[9px] font-black text-zinc-400 uppercase tracking-widest leading-none mb-2">Mingguan</p>
                            <p className="text-4xl font-black text-zinc-900 leading-none">{stats.weekly}</p>
                        </div>
                        <div className="bg-white p-6 rounded-[1.5rem] border border-zinc-200 shadow-lg shadow-zinc-200/30">
                            ...
                            <p className="text-4xl font-black text-zinc-900 leading-none">{stats.monthly}</p>
                        </div>
                        <div className="col-span-2 bg-zinc-900 p-6 rounded-[1.5rem] border border-zinc-800 shadow-xl">
                    ...
                    {/* HISTORY LIST MINI */}
                    <div className="bg-white rounded-[2rem] border border-zinc-200 shadow-xl shadow-zinc-200/30 overflow-hidden flex-1 overflow-y-auto max-h-[400px] custom-scrollbar">
                        ...
                        <div className="divide-y divide-zinc-50">
                            {history.slice(0, 10).map((h, i) => (
                                <div key={i} className="p-4 flex justify-between items-center bg-white hover:bg-zinc-50 transition-all">
                                    <div>
                                        <p className="text-sm font-black text-zinc-900 font-mono tracking-tighter leading-none mb-1">{h.bk}</p>
                                        <p className="text-[8px] font-black text-zinc-400 uppercase tracking-widest">{h.tipe}</p>
                                    </div>
                                    <p className="text-[10px] font-black text-zinc-400 font-mono">{new Date(h.completedAt).toLocaleDateString('id-ID', { day: 'numeric', month: 'short' })}</p>
                                </div>
                            ))}
                        </div>
                        {history.length === 0 && (
            ...
            {selectedUnit && selectedUnit !== 'settings' && (() => {
                ...
                return (
                    <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-[100] flex items-center justify-center p-4" onClick={() => setSelectedUnit(null)}>
                        <div className="bg-white rounded-[3rem] w-full max-w-2xl max-h-[90vh] shadow-2xl flex flex-col border border-zinc-100 animate-fade-in relative overflow-hidden" onClick={e => e.stopPropagation()}>
                            <div className="bg-zinc-900 p-8 border-b border-zinc-800 flex justify-between items-center relative overflow-hidden">
                                <div className="absolute top-0 right-0 w-32 h-32 bg-white/5 rounded-bl-full -z-10"></div>
                                <div className="flex items-center gap-6">
                                    <div className="bg-blue-600 p-3.5 rounded-2xl text-white shadow-xl shadow-blue-900/20"><Zap size={24} fill="white" /></div>
                                    <div>
                                        <h3 className="text-3xl font-black uppercase tracking-tighter text-white leading-none mb-1 font-mono">{liveUnit.bk}</h3>
                                        <p className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">{liveUnit.tipe} • Work Progress</p>
                                    </div>
                                </div>
                                <button onClick={() => setSelectedUnit(null)} className="p-3 bg-white/10 text-white/50 hover:text-white rounded-2xl transition-all border border-white/10"><X size={20} /></button>
                            </div>

                            <div className="flex-1 overflow-y-auto p-8 custom-scrollbar">
                                <div className="mb-8 p-6 bg-red-50/50 rounded-3xl border border-red-100 flex items-start gap-4">
                                    <div className="p-2 bg-red-500 rounded-lg text-white shrink-0"><Activity size={16} /></div>
                                    <div>
                                        <p className="text-[9px] font-black text-red-400 uppercase tracking-widest mb-1.5">Keluhan Utama</p>
                                        <p className="text-lg font-bold text-zinc-900 leading-tight italic">"{liveUnit.keluhan || 'Tidak ada catatan keluhan'}"</p>
                                    </div>
                                </div>

                                {/* CHECKLIST */}
                                <h4 className="text-[10px] font-black uppercase text-zinc-400 tracking-widest ml-1 mb-4 flex items-center gap-2">
                                    <div className="w-1.5 h-1.5 bg-emerald-500 rounded-full"></div> Progress Checklist
                                </h4>
                                <div className="space-y-3">
                                    {(liveUnit.checklist || []).length === 0 ? (
                                        ...
                                        <button
                                            ...
                                            className={`w-full flex items-center gap-4 p-5 rounded-2xl transition-all border shadow-sm ${task.completed ? 'bg-emerald-50 border-emerald-100 shadow-emerald-100' : 'bg-white border-zinc-100 hover:border-emerald-500'}`}
                                        >
                                            <div className={`w-8 h-8 rounded-lg flex items-center justify-center transition-colors ${task.completed ? 'bg-emerald-500 text-white' : 'bg-zinc-50 text-zinc-300'}`}>
                                                <CheckCircle size={18} />
                                            </div>
                                            <span className={`font-bold uppercase tracking-tight text-left ${task.completed ? 'text-emerald-700 line-through opacity-60' : 'text-zinc-900'}`}>{task.text}</span>
                                        </button>
                                    ))}
                                    {liveUnit.status !== 'working' && (
                                        <p className="text-center text-[10px] font-black text-zinc-400 mt-6 bg-zinc-50 py-3 rounded-xl border border-zinc-100 uppercase tracking-widest">Klik "Mulai Pekerjaan" untuk mengaktifkan Checklist</p>
                                    )}
                                </div>
                            </div>
                        </div>
            ...
            {selectedUnit === 'settings' && (
                <div className="fixed inset-0 bg-black/70 backdrop-blur-md z-[100] flex items-center justify-center p-4" onClick={() => setSelectedUnit(null)}>
                    <div className="bg-white rounded-[2.5rem] w-full max-w-md shadow-2xl p-8 border border-zinc-100 animate-fade-in relative overflow-hidden" onClick={e => e.stopPropagation()}>
                        ...
                        <form onSubmit={handlePasswordSubmit} className="space-y-5">
                            ...
                            <button type="button" onClick={() => setSelectedUnit(null)} className="w-full text-zinc-400 text-[10px] font-black uppercase tracking-widest py-2">Batal</button>
                        </form>
                    </div>
                </div>
            )}
