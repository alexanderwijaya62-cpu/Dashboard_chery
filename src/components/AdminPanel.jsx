            ...
            <header className="bg-white border-b border-zinc-200 px-3 md:px-6 py-1.5 flex justify-between items-center z-50 shrink-0 shadow-sm overflow-x-auto">
                ...
                <div className="flex items-center gap-6">
                    ...
                    <div className="relative">
                        ...
                        {showQueueSettings && (
                            <div className="absolute right-0 top-full mt-2 bg-white border border-zinc-200 rounded-2xl shadow-2xl p-4 z-50 min-w-[240px]">
                                <p className="text-[9px] font-black text-zinc-400 uppercase tracking-widest mb-3">Pengaturan Nomor Antrian</p>
                                <div className="flex items-center gap-3">
                                    <div className="flex-1">
                                        <label className="text-[8px] font-bold text-zinc-400 uppercase tracking-wider">Reguler Mulai Dari</label>
                                        <input type="number" min="2" max="99"
                                            value={regulerStartNum}
                                            onChange={e => setRegulerStartNum(parseInt(e.target.value) || 6)}
                                            className="w-full bg-zinc-50 border border-zinc-200 rounded-xl px-3 py-2 text-sm font-black text-black outline-none focus:border-black mt-1"
                                        />
                                    </div>
                                    <button onClick={saveRegulerStartNum}
                                        className="mt-5 px-4 py-2 bg-black text-white rounded-xl text-[9px] font-black uppercase tracking-wider hover:bg-zinc-800 transition-all active:scale-95">
                                        Simpan
                                    </button>
                                </div>
                                <p className="text-[8px] text-zinc-400 mt-2 font-bold">Booking otomatis 1-{regulerStartNum - 1}, Reguler mulai {regulerStartNum}</p>
                            </div>
                        )}
                    </div>
                </div>
