...
async function test() {
    ...
    try {
        ...
        let cookies = initialResp.headers.getSetCookie ? initialResp.headers.getSetCookie() : [];
        ...
        const newCookies = loginResp.headers.getSetCookie ? loginResp.headers.getSetCookie() : [];
        ...
        console.log("Response snippet:", body.slice(0, 1500));
    } catch (err) {
